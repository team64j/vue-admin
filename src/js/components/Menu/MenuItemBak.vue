...
export default {
  ...
  computed: {
    node () {
      ...
      } else if ((this.propData?.length && this.level) || this.data['url']) {
        slots.push(h('span', {
    ...
    classLI () {
      ...
      if (this.data['data']?.length || this.data['url']) {
        c += ' parent'
      }
...
<template>
  <li :data-level="level" :class="classLI" @mouseenter="mouseenter" @mouseleave="mouseout" @click="mouseclick">
